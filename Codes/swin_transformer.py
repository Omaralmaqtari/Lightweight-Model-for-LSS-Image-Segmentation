...
import numpy as np
from swin_heads import UPerHead, FCNHead
...
class SwinTransformerBlock(nn.Module):
    def __init__(self, dim, num_heads, window_size=7, shift_size=0, mlp_ratio=4., qkv_bias=True, qk_scale=None,
...
class SwinTransformer(nn.Module):
    def __init__(self, patch_size=4, in_chans=2, embed_dim=96, depths=[2, 2, 6, 2], num_heads=[3, 6, 12, 24],
...
class SwinTransformerSeg(nn.Module):
    def __init__(self, ch_in, ch_out):
        super(SwinTransformerSeg, self).__init__()
        
        self.encoder = SwinTransformer(embed_dim=96, in_chans=ch_in, depths=[2, 2, 6, 2], num_heads=[3, 6, 12, 24],
                                       window_size=7, mlp_ratio=4., qkv_bias=True, qk_scale=None, drop_rate=0.,
                                       attn_drop_rate=0., drop_path_rate=0.3, patch_norm=True,
                                       out_indices=(0, 1, 2, 3))
        
        self.decoder = UPerHead(in_channels=[96, 192, 384, 768], in_index=[0, 1, 2, 3], pool_scales=(2, 3, 4, 6),
                                channels=512, dropout_ratio=0.1, num_classes=16,
                                norm_cfg=dict(type='BN', requires_grad=True), align_corners=False)
        
        self.auxiliary_decoder = FCNHead(in_channels=384, in_index=2, channels=256, num_convs=1,
                                         concat_input=False, dropout_ratio=0.1, num_classes=16,
                                         norm_cfg=dict(type='BN', requires_grad=True), align_corners=False)
        
        self.out = nn.Conv2d(32, ch_out, kernel_size=1)
